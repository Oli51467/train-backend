<template>
    <ContentBase>
        ...
        <a-table :dataSource="trainCarriages" :columns="columns" :pagination="pagination" @change="handleTableChange"
            :loading="loading">
        ...
        <a-modal v-model:visible="visible" title="火车车箱" @ok="handleOk" ok-text="确认" cancel-text="取消">
            <a-form :model="trainCarriage" :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }">
                <a-form-item label="车次编号">
                    <TrainSelect v-model="trainCarriage.trainCode"></TrainSelect>
                </a-form-item>
    ...
    setup() {
        ...
        const handleQuery = (param) => {
            ...
            axios.get("/business/admin/train-carriage/getAll/", {
                ...
                if (data.success) {
                    let resp = data.data;
                    trainCarriages.value = resp.list;
                    // 设置分页控件的值
